...
/mob/human/Bumped(mob/M)
	if((istype(M, /mob/human))  && src.zombie)

		var mob/human/B = M
		if(prob(4) && !M.zombie)
			src << "As [B.rname] pushes you away, your zombie reflexes allow you to quickly attempt to bite him."
			B.attack_hand(src)


/mob/human/m_delay()
/*
	var/obj/item/weapon/organ/external/A = src.contents["r_leg"]
	var/obj/item/weapon/organ/external/B = src.contents["r_foot"]
	var/obj/item/weapon/organ/external/C = src.contents["l_leg"]
	var/obj/item/weapon/organ/external/D = src.contents["l_foot"]
*/
	var/tally = 0
	if(src.wear_suit)
...
/*		if (A.brute_dam > 35 || B.brute_dam > 35 || A.burn_dam > 35 || B.burn_dam > 35)
			if(!istype(src.r_hand, /obj/item/weapon/cane) || !istype(src.l_hand, /obj/item/weapon/cane))
				tally += 5
		if (C.brute_dam > 35 || D.brute_dam > 35 || C.burn_dam > 35 || D.burn_dam > 35)
		if(!istype(src.r_hand, /obj/item/weapon/cane) || !istype(src.l_hand, /obj/item/weapon/cane))
				tally += 5
		if (A.broken || B.broken)
			if(!istype(src.r_hand, /obj/item/weapon/cane) || !istype(src.l_hand, /obj/item/weapon/cane))
				tally += 15
		if (C.broken || D.broken)
			if(!istype(src.r_hand, /obj/item/weapon/cane) || !istype(src.l_hand, /obj/item/weapon/cane))
				tally += 15
*/
...
//	var/obj/item/weapon/organ/external/temp
	...
	if(src.check_burning() && src.alive())
...
/mob/human/las_act(flag, A as obj)
	...
	if ((shielded && flag != "bullet"))
		if (!flag)
			...
			src.unlock_medal("Who you gonna call?",1, "Get your shield disrupted by a laser beam." , "hard")
...
/mob/human/UpdateClothing()
	...
	if(is_rev == "no")
		src.overlays += image("icon" = 'other.dmi', "icon_state" = "[src.lying?"l":"s"]")
...
/mob/human/attack_hand(mob/human/M as mob)
	...
	if (M.gloves && M.gloves.elecgen == 1 && M.a_intent == "hurt")
		if(M.gloves.uses > 0)
			...
			return
	if (M.a_intent == "help" && M.is_living())
		if (src.health > 0)
			if (src.w_uniform)
