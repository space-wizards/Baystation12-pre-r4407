	...
	switch(wireIndex)
		if(AIRLOCK_WIRE_MAIN_POWER1 || AIRLOCK_WIRE_MAIN_POWER2)
			if ((!src.isWireCut(AIRLOCK_WIRE_MAIN_POWER1)) && (!src.isWireCut(AIRLOCK_WIRE_MAIN_POWER2)))
				...
				src.shock(usr, 50)
				src.updateUsrDialog()
		//		usr.unlock_medal("Shocking Situation",1,"Get electrocuted. Wear orange gloves next time.")
		if (AIRLOCK_WIRE_BACKUP_POWER1 || AIRLOCK_WIRE_BACKUP_POWER2)
...
/obj/machinery/door/airlock/attackby(C as obj, mob/user as mob)
	...
	else if (istype(C, /obj/item/weapon/chem/beaker))
		if (C:chem.chemicals.Find("thermite") && C:chem.chemicals.len == 1)
			var/obj/item/weapon/chem/beaker/V = C
			var/obj/substance/chemical/S = V:chem.split(20)
			if (S:volume() >= 20)
				var/turf/T = user.loc
				user << "\blue You carefully begin to affix the beaker to the airlock."
				sleep (100)
				if ((user.loc == T && user.equipped() == C && !( user.stat )) && istype(src, /turf/station/r_wall))
					user << "\blue You affix the beaker to the airlock."
					src.destroying = 8
					del (V)
					if (C)
						del (C)
