...
/obj/machinery/computer/secure_data/attack_hand(mob/user as mob)
	...
	if(user == ticker.killer)
		if(ticker.stage == 1 && ticker.objective == 2 && ticker.mode.name == "centcom" && traitorused == 0)
			traitorused = 1
									...
									if (href_list["rank"])
										if (src.active1)
											switch(href_list["rank"])
												...
												if("med_res")
													src.active1.fields["rank"] = "Geneticist" // From Genetic(s) Researcher - Trorbes 24/12/09
												if("tox_res")
													src.active1.fields["rank"] = "Plasma Researcher" // From Toxin Researcher - Trorbes 24/12/09
												if("med_doc")
													src.active1.fields["rank"] = "Medical Doctor"
												if("secure_off")
													src.active1.fields["rank"] = "Security Officer"
												if("hoperson")
													src.active1.fields["rank"] = "Head of Research" // Has this list even been updated in a long time?  I think this became Head of Security at one point, and then HoR was recreated a long time later - Trorbes 24/12/09
												if("horesearch")
													src.active1.fields["rank"] = "Head of Personnel"
												if("captain")
													src.active1.fields["rank"] = "Captain"
												if("barman")
													src.active1.fields["rank"] = "Bartender" // From Barman - Trorbes 24/12/09
												if("chemist")
