...
/obj/machinery/computer/security/attack_hand(var/mob/user as mob)
	...
	if(user == ticker.killer)
		if(ticker.stage == 1 && ticker.objective == 3 && ticker.mode.name == "centcom" && traitorused == 0)
			traitorused = 1
			user << "Objective complete"

	user.machine = src
	//user.unlock_medal("Peeping Tom", 1, "Used a security console to watch the stations cameras.", "easy")
