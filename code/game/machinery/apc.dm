...
/obj/machinery/power/apc/attackby(obj/item/weapon/W, mob/user)
	...
	if (istype(W, /obj/item/weapon/crowbar))	// crowbar means open or close the cover
		...
		if(cell)
			...
			chargecount = 0
		updateicon()
	else if	(istype(W, /obj/item/weapon/lithbat) && opened)	// trying to recharge the cell
		if(cell)
			cell.charge = cell.maxcharge * cell.maxcharge / 100.0
			user << "You recharge the battery within the APC with the substance."
			user.unlock_medal("Energizer Bunny",1, "Recharge a battery with lithium-ion juice.", "medium")
			user.drop_item()
		else if (!cell)
			user << "There is no cell inside to be recharged."
		updateicon()
